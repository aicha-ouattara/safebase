...
class Database
{
    ...
    public function dumpDatabase($databaseName, $HOST, $USERNAME, $PASSWORD){
        ...
        exec("mysqldump --user={$USERNAME} --password={$PASSWORD} --host={$HOST} --skip-ssl {$databaseName} --result-file={$dir} 2>&1", $output);
        return $output;
    }
    ...
    public function getCollectionDatabases()
    {
        ...
        if ($filteredDatabases === []) {
            // createDatabase() echoes a JSON status message as a side effect;
            // suppress it here so it doesn't get appended to this method's own
            // JSON response.
            ob_start();
